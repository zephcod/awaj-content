/**
 * Cron endpoint for the Instagram queue — for serverless deploys where
 * the in-process worker (instrumentation.ts) doesn't run. Point a cron
 * at GET /api/cron/ig every minute (see vercel.json).
 *
...
import { processDueIgPosts } from "@/lib/igqueue";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await processDueIgPosts();
